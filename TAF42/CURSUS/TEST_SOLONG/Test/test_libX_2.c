 ...
 * 	Description :												*
 *														*	
 *		1) Handler d'evenement pour la position et la touche de la souris : [mouse_hook]		*
 *														*
 *		2) Handler d'evenement pour la presence et la postion de la souris dans la 			*
 *		fenetre : [mouse_move()]									*
 *														*
 *			- code 1 : verifie si les coordonne de la souris son a l'interieur des dimensons 	*
 *			de la fennetre										*
 *														*
 * 		3) handler d'evenement pour le click sur la croix rouge : [handle_redcross_keypress()]		*
 *														*
 * 			- code 1 : detruit fenetre graphique cree par mlx_new_window()				*
 *														*
 *														*
 * 		4) handler d'evenement pour une touche appuye : [handle_keypress()]				*
 *														*
 ...
 *														*
 *														*
 * 		5)  handler d'evenemnt pour une touche relacher : [handle_keyrelease()]  			*
 *														*
...
int	mouse_hook(int p, t_data *data) /* 1 */
{
	printf("Touche de souris : Num [%d]\n", p);

	return 0;
}

int	mouse_move(int x, int y, t_data *data) /* 2 */
{
...
int	handle_redcross_keypress(t_data *data) /* 3 */
{
...
int	handle_keypress(int keysym, t_data *data) /* 4 */
{
	if (keysym == XK_Escape) 
	{
		mlx_destroy_window(data->mlx, data->mlx_win); 
...
int	handle_keyrelease(int keysym, void *data) /* 5 */
{
	printf("Keyrelease: [%d]\n", keysym); 
...
int	pikatchu_put(t_data *data)
{
	if (data->mlx_win == NULL)
		return 1;
	
	data->path_pikatchu_img = "../xpm/pikaFace.xpm"; 

	data->img.mlx_img = mlx_xpm_file_to_image(data->mlx, data->path_pikatchu_img, &data->img_w, &data->img_h);
...
int	main(void)
{
	...
	//mlx_loop_hook(data.mlx, &pikatchu_put, &data); /* 6 */
	mlx_hook(data.mlx_win, KeyPress, KeyPressMask, &handle_keypress, &data); /* 7 */
	mlx_key_hook(data.mlx_win, handle_keyrelease, &data); /* 7 */
	...
	//mlx_destroy_image(data.mlx, data.img_pika); /* 10 */
